...
if(isset($_POST['submit']))
{
	...
	{
		$sql = "UPDATE package set branch_id='$_POST[branch_id]',vehicle_type='$_POST[vehicle_type]',package_title='$_POST[package_title]',package_cost='$_POST[package_cost]',total_km='$_POST[total_km]', package_type='Driving Class',note='$_POST[note]',status='$_POST[status]',no_of_days='$_POST[no_of_days]' WHERE package_id='$_GET[editid]'";
	...
	{
		$sql = "INSERT INTO package(branch_id,vehicle_type,package_title,package_cost,total_km, package_type,note,status,no_of_days) values('$_POST[branch_id]','$_POST[vehicle_type]','$_POST[package_title]','$_POST[package_cost]','$_POST[total_km]','Driving Class','$_POST[note]','$_POST[status]','$_POST[no_of_days]')";
		$qsql = mysqli_query($con,$sql);
		echo mysqli_error($con);
		if(mysqli_affected_rows($con) == 1)
		{
			echo "<script>alert('Package record inserted successfully...');</script>";
			echo "<script>window.location='drivingclasspackage.php';</script>";
		}
	}
}
?>
...
<div class="item">
	<div class="serviceBox">
		<h3 class="title">Driving Class Package</h3>
		<p class="description">
...
<div class="row">
	<div class="col-md-4" style="padding-top: 7px;"	>Total KM</div>
	<div class="col-md-8">

<input type="text" name="total_km" id="total_km" class="form-control" value="<?php echo $rsedit['total_km']; ?>">
<span id="errtotal_km" class="errorclass" ></span>
	</div>
</div>
<br>

<div class="row">
	<div class="col-md-4" style="padding-top: 7px;"	>No. of days (Maximum)</div>
	<div class="col-md-8">

<input type="number" name="no_of_days" id="no_of_days" class="form-control" value="<?php echo $rsedit['no_of_days']; ?>">
<span id="errno_of_days" class="errorclass" ></span>
	</div>
</div>
<br>
...
function validateform()
{
	...
	if(document.getElementById("total_km").value=="")
	{
		document.getElementById("errtotal_km").innerHTML="Total km should not be empty..";
		i=1;
	}
	if(document.getElementById("no_of_days").value=="")
	{
		document.getElementById("errno_of_days").innerHTML="Number of days should not be empty..";
		i=1;
	}
